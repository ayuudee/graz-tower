...
/**
 * fn-14.1 (G3a-react R11) — hysteresis via mission-tree rewrite.
 *
 * Distinct from fn-12.2 (where `pendingAtcGoAroundFrom` flag was
 * needed because the ATC instruction arrived asynchronously) and
 * fn-13.1 (where `continueApproachIssuedThisAttempt` witness was
 * needed because CONTINUE APPROACH does NOT rewrite the mission
 * tree). G3a-react's hysteresis is **free**: the rewrite to
 * `CircuitAfterGoAround` takes `currentStep` out of the crosswind
 * recognition step-set, so the second decision cycle with the same
 * wind state produces zero events.
 *
 * Two-cycle scenario:
 *  1. Cycle 1: aircraft on FLY_FINAL, wind 20 kt crosswind, POH 15 kt.
 *     derivePilotEvent → CrosswindLimitExceeded fires.
 *     applyCrosswindGoAround → mission rewritten to CircuitAfterGoAround,
 ...
 *     `currentStep == GOING_AROUND` is not in {FLY_FINAL, REPORT_FINAL,
 *     AWAIT_LANDING_CLEARANCE, LAND}.
 *
 * The pin protects against a regression that (a) widens the eligible
 * step set to include GOING_AROUND or (b) breaks the subtree rewrite
 * so that `currentStep` stays in the eligible set.
 */
class PilotCrosswindHysteresisTest {
    ...
    fun `cycle 1 fires event + rewrites tree, cycle 2 returns null because currentStep is outside the recognition set`() {
        ...
        // Wind 20 kt pure crosswind vs C172's 15 kt POH.
        val wind = availableWind(directionDegrees = 360, speedKnots = 20)

        // Cycle 1: event fires.
        val cycle1Event = derivePilotEvent(aircraft(), initial, wind)
        assertTrue(
            cycle1Event is PilotEvent.CrosswindLimitExceeded,
            "cycle 1: 20 kt crosswind > 15 kt POH must fire CrosswindLimitExceeded; got $cycle1Event",
        )
        // Apply the GA — mission tree rewrite + reset.
        val gaResult = applyCrosswindGoAround(cycle1Event, initial, aircraft(), now0)
        ...
        // must return null — GOING_AROUND is not in the crosswind
        // eligible step set.
        ...
        assertNull(
            ...
            "cycle 2: hysteresis via tree-rewrite — currentStep=GOING_AROUND is outside the eligible " +
                "set so no event fires even though wind still exceeds the POH limit; got $cycle2Event",
        )
    }
}
