...
**Contract:** An `AirportProcedure.requiresStartupClearance: Boolean` field on the airport manifest, populated from real-world data. `groundDepartureTask(airport)` returns a tree containing `REQUEST_STARTUP` and `AWAIT_STARTUP_APPROVAL` iff the airport requires it. A new `CLEARANCE_DELIVERY` controller role, with a `ClearanceDeliveryStage` and `clearanceDeliveryProcedure()` analogous to `groundTaxiProcedure()`, issues `StartupApproved` in response to `Request(RequestStartup)`. The mission tree branch is determined by airport, never by cockpit type. Closing this also provides orderable evidence that `StartupApproved` occurred before an explicit engine-start observation; mission-step completion and default `AircraftState.engineRunning == true` are not valid substitutes.
...
**Status:** closed
**Closed by:** fn-28 epic (fn-28.8 abort foundation + fn-28.9 abort pilot branch + sim golden) — V1/Vr-style decision modelled as the **pilot-side abort recognition** firing pre-rotation on the typed `AircraftState.engineRunning: Boolean` ground-truth field (round-2 Major 4 — scalar Boolean, not the original `EngineState` sealed-type design; the simpler shape carries the same fn-28.9 abort recognition load-bearing on `engineRunning == false`; future engine-state granularity — LowPower, partial-failure — can extend the field as a sibling decision).
**Enforcement:** `pilot/.../AircraftState.kt::engineRunning: Boolean` ground-truth field (read by physics + abort recognition only — KDoc-documented firewall contract); `sim/.../SimEvent.kt::EngineFailure(aircraftId, time, seq, source = AgentId.System)` body-declared val per fn-28.8 round-1 codex fix (callers cannot mis-author the source); `sim/.../Step.kt::handleEngineFailure` loud-fail on unknown aircraft + idempotent on already-failed (no synthetic wake event per round-2 Major 4); `pilot/.../InstructorInput.kt::EngineFailureAt(aircraftId, time)` typed cockpit-side briefing input + `sim/.../testing/InstructorBriefing.kt::toInitialEvents(baseSeq)` + `withFixtureBaseSeqAdvanced` test-scaffolding translator pair; `sim/.../Step.kt::advanceKinematics` R12 engine-off clamp (decel allowed; accel blocked; bounded by `min(targetSpeedMps, currentSpeedMps)` when `engineRunning == false`) — pinned in `sim/.../EngineOffKinematicClampSpec.kt`; `pilot/.../PilotMission.kt::MissionStep.ABORTED` + `CompletionMode.NON_COMPLETING` runway-terminal pair (R15 + R20 audits at `PilotCognitive.isStepComplete` / `isReportComplete` / `stepTransmission` / `Pilot.planRoute`); `pilot/.../PilotMission.kt::isAbortTakeoffEligible(mission)` named guard (R16 split — takeoff-roll mission shapes `AWAIT_TAKEOFF_CLEARANCE` / `FLY_DEPARTURE`, distinct from fn-28.2's `isDensityAltitudeDeclineEligible`); `pilot/.../observe/PilotEvent.kt::AbortTakeoff(aircraftId, speedAtFailure)` leaf + `deriveAbortTakeoffEvent` branch with 4-check gate (`engineRunning == false` + `speedMps < rotationSpeedMps` strict + `phase == TakeoffRoll` v1 on-runway proxy + `isAbortTakeoffEligible(mission)`); R21 final branch order `DecisionAltitudeWithoutClearance → DensityAltitudeDecline → AbortTakeoff → TailwindLimitExceeded → CrosswindLimitExceeded` locked; `pilot/.../Pilot.kt::applyAbortTakeoff` with R13 sole-rewrite primitive `replaceFromActivePrimitive([PrimitiveTask(ABORTED, NON_COMPLETING)])` + at-rest intent (`targetSpeedMps = 0`, `route = None`, `altitudeM = 0`) + `suppressSameTickCognitive = true` payload (R14 round-15 Major 2 — covers ALL `pilotDecide` return paths via the shared `applyCognitiveSuppression` focused seam); `pilot/src/commonTest/.../IsAbortTakeoffEligibleSpec.kt` MissionStep enumeration (eligible: AWAIT_TAKEOFF_CLEARANCE + FLY_DEPARTURE; pre-taxi / airborne / arrival / terminal NON_COMPLETING all fail closed); `pilot/src/commonTest/.../observe/PilotEventAbortTakeoffTest.kt` 4-check gate matrix (engineRunning + pre-VR strict + phase + mission shape) + per-type rotation-speed parity (C172 vs B738); `pilot/src/commonTest/.../PilotAbortTakeoffTest.kt` apply post-conditions (R13 rewrite, R20 NON_COMPLETING, R14 suppression, defensive fail-closed); `sim/src/jvmTest/.../G0AbortTakeoffEngineFailureTest.kt` end-to-end sim golden — **eighth reactive-GA-class path / first emergency-event anchor** (positive scenario: dynamic `EngineFailureAt(t_CTO + 1ms)` injection via `runUntilWithStateTraceAndInjection`'s `EventInjection` hook — observes `ClearedForTakeoff` then injects; three-layer pin: kinematic instant-stop via R12 clamp + targetSpeedMps=0; mission-tree rewrite to ABORTED NON_COMPLETING with R20 invariants; never-airborne with altitudeM=0 / phase=TakeoffRoll preserved. Negative scenario: post-rotation injection — abort gate fails on speed predicate; test ends after gate-assertion per round-2 Major 7 — no further ticks, no recovery flow modelled at fn-28); `sim/.../testing/Fixtures.kt::LOWG_ABORT_TAKEOFF_PRE_VR` + `LOWG_ABORT_TAKEOFF_POST_VR` fixture aliases (base scenario data ONLY; dynamic injection lives in test method per round-7 Minor 3); `sim/.../testing/RunUntil.kt::runUntilWithStateTraceAndInjection` + `EventInjection` driver variant for dynamic event injection from the post-step hook.
...
### D-AUDIT.15-FOLLOWUP — Cognitive-pilot repetition recovery after stepped-on controller transmissions
**Status:** narrative
**Pinned at:** narrative only
**Why:** fn-50 models final reception quality and a minimal non-cognitive pilot `SayAgain` recovery path for ICAO 9432 §2.8.1.4, but enabling automatic repetition recovery in full cognitive missions currently perturbs LOWG golden-style timing and can create repeated radio congestion. The cognitive path needs an explicit design for outstanding repetition state, controller repeat scheduling, and mission interaction before it is enabled globally.
**Closes by:** new epic when cognitive-mission radio recovery becomes test-driven.
...
**Enforcement:** `assertExpectedLjmbIfrSids` helper body in `migration/src/jvmTest/kotlin/xyz/easiersaid/twr/migration/world/LjmbWorldCandidateValidationTest.kt` reshaped to a structural-validity + missing-only-coverage decomposition (`assertSidsAreStructurallyValid` + `assertSidsCoverPromisedSet`) against the current 5-SID CIFP-projectable set (`LJMB_SID_GOLV2G_14, LJMB_SID_PETO2B_14, LJMB_SID_PETO5D_32, LJMB_SID_VALU1S_14, LJMB_SID_VALU4L_32` — `cycle unknown; source data/cifp/LJMB.dat at git-sha a28fb1eed6a0ff80aedd0a3f3336a35f50d66a97`). The four stale `*1S` SIDs (`DIML1S, GOLV1S, MURE1S, PETO1S`) were from an older AIRAC cycle that no longer exists in `data/cifp/LJMB.dat`. Auto-regenerated `cad/airports/rendered/ljmb/world-validation-report.json` matches the test's first-green output (`issueCount = 0`, `structuralIssueCount = 0`). `./gradlew :migration:allTests --offline --no-daemon` green; `./gradlew build --offline --no-daemon` green; cross-epic carve-outs in fn-5 / fn-6.2 / fn-9.2 / fn-11.2 / fn-16.1 / fn-16.2 superseded. The 7 fixless-leg-blocked SIDs (`DIML1J, DIML2G, DIML3H, GOLV3H, VALU1N, VALU3G, VALU3H`) remain pending `.plan` M1 (`VI` heading-to-altitude leg support in the runtime waypoint route model); the new structural check will accept them automatically once M1 lands.
...
**Status:** closed
**Closed by:** fn-28 epic (fn-28.4 controller-side foundation + fn-28.5 sim golden) — reframed model per plan-review-round-1: GA + ATC sequencing, NOT simultaneous pilot recognition (pilot-side recognition is independent of ATC; B on downwind is rejected by `derivePilotEvent`'s final-phase guard so B never has its own wind-GA; the controller observes A's `Report(GoingAround)` and emits `ExtendDownwind` to B; when A's recovery `Report(Downwind)` clears the belief, `ARR-TURN-BASE` re-enables for B and `TurnBase(B)` fires same-cycle, with the existing `SupersessionRelation(TurnBase, ExtendDownwind, ABANDON)` row dropping B's prior ExtendDownwind coordination).
**Enforcement:** `controller/.../observe/BeliefState.kt::goAroundInProgressByRunway: Map<RunwayId, GoAroundInProgress>` persistent runway-scoped belief slice (R23 — round-7 Major 3, lives on BeliefState not ControllerView so the entry survives across cycles); `controller/.../observe/Observe.kt::resolveGoAroundRunway(aircraft, beliefs)` Either-returning runway-resolution helper (round-9 Major 1 fail-closed contract: TOWER_ARRIVAL `commitment.runway` primary path + `BeliefState.activeRunway` fallback per round-10 Major 3); `controller/.../observe/Observe.kt::BeliefState.withGoAroundInProgress(events, now)` fold (SET on `ControllerEvent.GoAroundDetected` with first-writer-wins tie-breaking per R23 round-7 Minor 1; CLEAR on pattern-rejoin `Report(Downwind/Final/Base)` with `receivedAt > setAtTime` strict-inequality per round-13 Major 3; CLEAR on 60s deterministic timeout `BeliefState.GO_AROUND_TIMEOUT_MS`; NO runway-vacate clause per round-7 Major 4 / round-8 Major 3); `controller/.../bdi/Guard.kt::GoAroundInProgressOnRunway` RuleGuard (reads the BeliefState slice — single read-site for the `ARR-EXTEND-FOR-GA` + `ARR-TURN-BASE` rules); `controller/.../procedure/TowerArrival.kt` new `ARR-EXTEND-FOR-GA` rule at AwaitApproach (guard: `OnCircuitLeg(DOWNWIND) AND GoAroundInProgressOnRunway AND NoPendingReadback`, action: `ExtendDownwindAction`; controller-observable predicates only — no `PilotPhase` reads per round-13 Major 2 firewall) + `ARR-TURN-BASE` gated with `Not(GoAroundInProgressOnRunway)` (concrete cancel-output: when belief clears, guard passes again and TurnBase fires same-cycle per round-10 Major 2); `controller/.../bdi/Supersession.kt:69` existing `SupersessionRelation(TurnBase::class, ExtendDownwind::class, PendingReadbackPolicy.ABANDON)` row consumes B's pending ExtendDownwind coordination — NO new supersession row needed; `controller/.../FirewallBeliefWriteTest.kt` allowlist extended for the new write site; `protocol/.../RegulationDatabase.kt::ICAO4444_12_3_4` (17th ed. Ch 12 §12.3.4 — Aerodrome Control Phraseologies / sequencing); `controller/src/commonTest/.../GoAroundSequencingSpec.kt` 13 unit tests covering: `resolveGoAroundRunway` 3 paths (commitment primary + activeRunway fallback + fail-closed), `withGoAroundInProgress` SET / fail-closed / first-writer-wins tie-breaking / CLEAR-on-pattern-rejoin / CLEAR-on-60s-timeout / same-cycle-stale-Final-does-NOT-clear / cross-aircraft-rejoin-does-NOT-clear, `ARR-EXTEND-FOR-GA` 3 firings (positive with trailing downwind + GA active; negative with no GA; round-trip via same-cycle GoAroundDetected fold), `ARR-TURN-BASE` blocked while GA active; `sim/src/jvmTest/.../G3aPilotReactiveMultiAircraftTest.kt` sim integration golden (Scenario 1 — crosswind GA + extend-downwind to B; three-layer pin: A's Report(GoingAround) + stage regression + ExtendDownwind(B) post-GA + B never declares wind-GA + B no-TurnBase during GA-active window; Scenario 2 — tailwind GA + extend-downwind to B with the same shape; Scenario 3 — GA-recovery via A's recovery-circuit `Report(Downwind)` clearing belief → TurnBase(B) fires after the recovery report → ordering pin proves TurnBase(B) < first Report(RunwayVacated) from A, confirming the R23 lifecycle is pattern-rejoin-driven NOT vacate-driven; determinism pin compares two runs from the same seed to surface any non-deterministic EVENT_ORDER fold ordering per round-8 Minor 1).

### D-PASS-g3a-react-multi-aircraft-tailwind — Multiple aircraft on same runway when wind shifts to tailwind
**Status:** closed
**Closed by:** fn-28 epic (fn-28.4 controller-side foundation + fn-28.5 sim golden) — reframed model per plan-review-round-1: GA + ATC sequencing, NOT simultaneous pilot recognition. Sibling closure of D-PASS-g3a-react-multi-aircraft-crosswind on the tailwind axis. Scenario 2 of `G3aPilotReactiveMultiAircraftTest` exercises the tailwind GA on A + controller-side `ARR-EXTEND-FOR-GA` to B; the controller-side machinery (BeliefState slice + fold + guards + rules) is **axis-agnostic** — the same `ARR-EXTEND-FOR-GA` rule fires regardless of whether A's GA was crosswind-triggered or tailwind-triggered (the `GoAroundInProgressOnRunway` guard reads the runway-scoped belief which is set on any `Report(GoingAround)` reception).
**Enforcement:** same controller-side machinery as the crosswind sibling (see D-PASS-g3a-react-multi-aircraft-crosswind for the full enumeration); `sim/src/jvmTest/.../G3aPilotReactiveMultiAircraftTest.kt::Scenario 2 (tailwind GA on A triggers ExtendDownwind to B and B holds downwind through GA-active window)` adds the tailwind-axis sim coverage; the controller-side `ARR-EXTEND-FOR-GA` fires identically on both axes because the belief lifecycle is driven off `Report(GoingAround)` reception (the recognition-axis-agnostic radio observable).

### D-PASS-g3b-react-cross-aerodrome-crosswind — Cross-aerodrome crosswind go-around at LJMB
**Status:** closed
**Closed by:** fn-28 epic (fn-28.6 Transit-arrival reactive-GA pipeline foundation + fn-28.7 G3b sim golden) — Transit-arrival reactive-GA path landed via the existing `applyCrosswindGoAround` dispatch fork (no new event leaf, no new applier, no destination-GA placeholder enum value).
**Enforcement:** `pilot/.../Pilot.kt::isTransitArrivalReactiveGoAroundEligible(aircraft, mission)` named guard (R18; data-honest predicate parts — Transit goal + Transit root-name gate per codex round-1 + wind-reactive eligible step + Final phase + active runway + flat-shape `activeCompound() == null` no-refire invariant); `pilot/.../observe/PilotEvent.kt::deriveCrosswindEvent` widened to disjunctive eligibility `isReactiveGoAroundEligible(mission) || isTransitArrivalReactiveGoAroundEligible(aircraft, mission)` (round-12 Major 1 — recognition is in `derive*Event`, NOT inside appliers per round-16 Major 1); `pilot/.../Pilot.kt::applyCrosswindGoAround` Transit-dispatch fork to shared `applyTransitArrivalReactiveGoAround` helper using `replaceFromActivePrimitive(listOf(goAroundTask(), circuitTask(), groundArrivalTask()))` (R13 + R22 — existing GA TaskNodes, NO destination-GA placeholder enum/string per round-5 Critical 2) + `mission.resetForGoAround(now).copy(root = rewrittenRoot)` pre-rewrite reset placement (round-16 Major 2) + R19 Tick A intent `climbSpeedMps + Final + None + patternAltitude`; `pilot/.../Pilot.kt::planRoute` Transit-cruise discriminator (R14 round-14 Major 1 — treat `Transit + FLY_DEPARTURE` as cruise ONLY when active inner compound is null, so post-suffix-replace recovery falls through to the GA-route special cases); `pilot/src/commonTest/.../IsTransitArrivalReactiveGoAroundEligibleSpec.kt` guard-shape matrix (Transit + step set + runway + phase + no-refire post-apply + null currentTask + root-name regression); `pilot/src/commonTest/.../PilotTransitArrivalReactiveGoAroundTest.kt` composition pin (both axes exercise the dispatch fork, R19 Tick A intent, R22 suffix shape, completed-prefix preservation, `resetForGoAround` cleared fields, no-refire invariant, circuit-only path unaffected for Arrival missions); `sim/src/jvmTest/.../G3bCrossAerodromeReactiveTest.kt::world-authored crosswind exceedance at LJMB triggers Transit-arrival reactive GA in cross-aerodrome flight` end-to-end sim coverage on the **seventh reactive-GA path / first cross-aerodrome reactive-GA path** (three-layer pin: exactly-one Report(GoingAround) between LJMB wind-shift and wind-recovery cycles + LJMB_TWR commitment-stage regression `{LandingClearanceIssued | AwaitLandedObserved} → AwaitDownwind` post-GA-radio-delivery + within-window kinematic non-event "aircraft never lands at LJMB" — recovery landing out-of-window per round-10 Minor 3 bounded-window discipline + R22 mission-tree shape pin asserting `GOING_AROUND` precedes recovery `FLY_DEPARTURE` in the post-shift step sequence); `sim/src/jvmTest/.../testing/Fixtures.kt::LOWG_LJMB_VFR_REACTIVE` cross-aerodrome reactive fixture variant (concrete LJMB OAT 13.27 °C + QNH 1013 hPa so the `PilotWiring.buildPilotInput` projection surfaces LJMB weather; initial wind 140°@10 pure-headwind on runway 14). Sibling closure of D-PASS-g3b-react-cross-aerodrome-tailwind on the tailwind axis — same machinery, same dispatch fork, same suffix-replace primitive.

### D-PASS-g3b-react-cross-aerodrome-tailwind — Cross-aerodrome tailwind go-around at LJMB
**Status:** closed
**Closed by:** fn-28 epic (fn-28.6 Transit-arrival reactive-GA pipeline foundation + fn-28.7 G3b sim golden) — sibling closure of D-PASS-g3b-react-cross-aerodrome-crosswind on the tailwind axis. The Transit-arrival reactive-GA dispatch fork in `applyTailwindGoAround` is **axis-agnostic** w.r.t. the cross-aerodrome surface — the same `isTransitArrivalReactiveGoAroundEligible(aircraft, mission)` guard, the same `applyTransitArrivalReactiveGoAround` shared helper, the same `replaceFromActivePrimitive([goAroundTask(), circuitTask(), groundArrivalTask()])` R22 suffix-replace, the same R19 Tick A intent fire regardless of whether the wind exceedance was crosswind- or tailwind-triggered. The recognition side (`deriveTailwindEvent`) carries the same widened disjunctive eligibility as the crosswind sibling (round-12 Major 1).
**Enforcement:** same Transit-arrival reactive-GA machinery as the crosswind sibling (see D-PASS-g3b-react-cross-aerodrome-crosswind for the full enumeration); `pilot/.../observe/PilotEvent.kt::deriveTailwindEvent` widened to `isReactiveGoAroundEligible(mission) || isTransitArrivalReactiveGoAroundEligible(aircraft, mission)`; `pilot/.../Pilot.kt::applyTailwindGoAround` Transit-dispatch fork mirrors the crosswind branch; `sim/src/jvmTest/.../G3bCrossAerodromeReactiveTest.kt::world-authored tailwind exceedance at LJMB triggers Transit-arrival reactive GA in cross-aerodrome flight` adds the tailwind-axis sim coverage (same three-layer pin shape as the crosswind axis; transition-2 recovery gate uses the post-GA recovery `Report(Downwind)` transmission per the inherited G3a-react-tailwind codex round-2 strengthening — pure radio observable, no peek into pilot state); the per-type doctrinal severity asymmetry (C172 AFH-advisory 10 kt vs B738 FCOM hard-limit 15 kt) surfaced in `AircraftType.maxTailwindKnots` KDoc is carried unchanged into the cross-aerodrome scenario — G3b exercises the C172 leaf only.
...
**Enforcement:** `pilot/src/commonMain/kotlin/xyz/easiersaid/twr/pilot/world/PilotAviationWorld.kt` defines `PilotAviationWorld` / `PilotAerodrome` / `PilotRunway` parallel data classes that omit `Aerodrome.weather` and `Runway.obstruction` (the dynamic entity fields). `AviationWorld.toPilotView()` extension fn projects at the firewall boundary using exhaustive named-argument constructor wiring (R4 — projection-construction-site gate: if a field is added to a projection type and a `toPilotView` call site forgets to wire it, that call site fails to compile). The complementary core-vs-projection future-field gate is `FirewallPilotAviationWorldTest`'s reflection-driven property-set parity assertion (R8 #2/#3) — that test fails fast when `:core/Aerodrome` or `:core/Runway` gains a field not mirrored on the projection (or vice versa). `PilotInput.world: PilotAviationWorld` (was `AviationWorld`); `sim/.../PilotWiring.kt` `buildPilotInput` calls `state.world.toPilotView()`. Pilot-side `Pilot.kt` (7+ signatures) and `PilotRoutePlanner.kt` (14+ signatures) migrated to `PilotAviationWorld`; pilot test fixtures (PilotAgentTypeSpec, FirewallPilotInputTest, PilotCrosswindTickATickBTest, PilotTailwindTickATickBTest, PilotAtcInitiatedGoAroundSpec, PlannedGoAroundSpec, PerTypeCircuitSpec, TransitRoutePlanningSpec) project via `.toPilotView()`. `pilot/jvmTest/.../FirewallPilotAviationWorldTest.kt` reflection-asserts seven properties: (1) no `weather` on `PilotAerodrome`; (2) no `obstruction` on `PilotRunway`; (3) `PilotAerodrome` property set == `Aerodrome` minus `{weather}`; (4) `PilotRunway` property set == `Runway` minus `{obstruction}`; (5) `PilotAviationWorld.aerodromes` value type == `PilotAerodrome`; (6) `PilotAerodrome.runways` value type == `PilotRunway`; (7) `PilotAviationWorld` top-level property set == `AviationWorld` top-level property set (the bidirectional future-field gate). Together: named-arg wiring stops sloppy projection construction; the parity tests catch core/projection drift. Reading `world.aerodromes[id].weather` or `world.aerodromes[id].runways[id].obstruction` from pilot code now FAILS TO COMPILE. fn-12 (`Runway.obstruction`) + fn-16 (`Aerodrome.weather`) rich-world-domain dynamic-state precedents codified in the type system; convention-via-KDoc replaced by structural enforcement.
