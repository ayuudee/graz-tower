...
**Enforcement:** `pilot/src/commonMain/kotlin/xyz/easiersaid/twr/pilot/world/PilotAviationWorld.kt` defines `PilotAviationWorld` / `PilotAerodrome` / `PilotRunway` parallel data classes that omit `Aerodrome.weather` and `Runway.obstruction` (the dynamic entity fields). `AviationWorld.toPilotView()` extension fn projects at the firewall boundary using exhaustive named-argument constructor wiring (R4 — future field additions to `:core/Aerodrome` or `:core/Runway` fail to compile at the projection's constructor call site). `PilotInput.world: PilotAviationWorld` (was `AviationWorld`); `sim/.../PilotWiring.kt` `buildPilotInput` calls `state.world.toPilotView()`. Pilot-side `Pilot.kt` (7+ signatures) and `PilotRoutePlanner.kt` (14+ signatures) migrated to `PilotAviationWorld`; pilot test fixtures (PilotAgentTypeSpec, FirewallPilotInputTest, PilotCrosswindTickATickBTest, PilotTailwindTickATickBTest, PilotAtcInitiatedGoAroundSpec, PlannedGoAroundSpec, PerTypeCircuitSpec, TransitRoutePlanningSpec) project via `.toPilotView()`. `pilot/jvmTest/.../FirewallPilotAviationWorldTest.kt` reflection-asserts seven properties: (1) no `weather` on `PilotAerodrome`; (2) no `obstruction` on `PilotRunway`; (3) `PilotAerodrome` property set == `Aerodrome` minus `{weather}`; (4) `PilotRunway` property set == `Runway` minus `{obstruction}`; (5) `PilotAviationWorld.aerodromes` value type == `PilotAerodrome`; (6) `PilotAerodrome.runways` value type == `PilotRunway`; (7) `PilotAviationWorld` top-level property set == `AviationWorld` top-level property set (R8 — bidirectional future-field gate complementing R4's named-arg construction gate). Reading `world.aerodromes[id].weather` or `world.aerodromes[id].runways[id].obstruction` from pilot code now FAILS TO COMPILE. fn-12 (`Runway.obstruction`) + fn-16 (`Aerodrome.weather`) rich-world-domain dynamic-state precedents codified in the type system; convention-via-KDoc replaced by structural enforcement.
