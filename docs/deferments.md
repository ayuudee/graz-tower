...
**Why:** v1 of G3a-react authored weather via `SimState.weatherByAerodrome: Map<AerodromeId, WeatherObservation>` (in `:sim/commonMain`) because `Aerodrome.weather` didn't exist as a typed slice on `:core`'s `Aerodrome` entity. Weather should live on the entity it describes (METAR is aerodrome-scoped); the flat-map shape is a pre-`project_rich_world_domain.md` artefact that survived only because no consumer was demanding entity-shape. Migration is hard-cutover and cross-cutting; ~25 test files reference `weatherByAerodrome` (mostly as `SimState.initial(..., weatherByAerodrome = mapOf(...) ...)` initializer-parameter callers; those keep working transparently because the constructor handles the fold).
**Contract:** Delete `SimState.weatherByAerodrome` field; add `val weather: WeatherObservation? = null` to `Aerodrome` in `core/.../world/WorldModel.kt`; relocate `WeatherObservation` from `:controller/ControllerTypes.kt` to `:core` (`:core` cannot import `:controller`; dep direction is the opposite); `SimState.initial(...)` keeps its `weatherByAerodrome: Map<AerodromeId, WeatherObservation>` parameter for caller ergonomics but **folds it into the world** at construction time via a new `AviationWorld.updateAerodrome(id) { transform }` lens helper (mirrors fn-12's `RunwayObstructionWiring` precedent); migrate three production readers (`PilotWiring.buildPilotInput`, `ControllerWiring.buildControllerView`, `SimTraceQueries.weatherTransitions`) to walk `world.aerodromes` + `G3aPilotReactiveCrosswindTest.kt:797-802` mutator (changes from `st.copy(weatherByAerodrome = ...)` to a `world.updateAerodrome(id) { it.copy(weather = ...) }` write) + **8 direct-constructor `SimState(...)` test sites** (those that bypass `SimState.initial`). ~25 `SimState.initial(...)` fixture callers do NOT migrate — the fold happens inside the constructor, so initializer-parameter callers keep working transparently. **Firewall surfaces UNCHANGED**: `PilotInput.weatherByAerodrome: Map<AerodromeId, WindReport>` (fn-14.1) keeps its shape; `ControllerView.weather: WeatherObservation?` keeps its shape — only the *source* changes (readers walk `world.aerodromes` instead of `state.weatherByAerodrome`). **Invariants**: existing `MissingWeatherForRunwayAerodrome` stays in `SimState.initial` (now checks the post-fold world: `aerodrome.runways.isNotEmpty() && aerodrome.weather == null`); NEW invariant `WeatherForUnknownAerodrome` added in `SimState.initial` to fail-fast on typo'd ids before the fold (closes the silent-drop window from `updateAerodrome`'s no-op-on-absent-id behaviour).
**Closes by:** fn-16 (fn-16.1 lands the foundation + atomic-cutover field migration; fn-16.2 closes the epic with the test-fixture-sweep + KDoc pass).
