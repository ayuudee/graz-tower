...
**Why:** v1 of G3a-react authored weather via `SimState.weatherByAerodrome: Map<AerodromeId, WeatherObservation>` (in `:sim/commonMain`) because `Aerodrome.weather` didn't exist as a typed slice on `:core`'s `Aerodrome` entity. Weather should live on the entity it describes (METAR is aerodrome-scoped); the flat-map shape is a pre-`project_rich_world_domain.md` artefact that survived only because no consumer was demanding entity-shape. Migration is hard-cutover and cross-cutting (25 test files construct via the flat map; production sim wiring + readers + fixtures all migrate atomically).
**Contract:** Delete `SimState.weatherByAerodrome`; add `val weather: WeatherObservation? = null` to `Aerodrome` in `core/.../world/WorldModel.kt`; relocate `WeatherObservation` from `:controller/ControllerTypes.kt` to `:core` (`:core` cannot import `:controller`); fold initial weather into `AviationWorld.aerodromes[*].weather` via a new `AviationWorld.updateAerodrome(id) { transform }` lens helper (mirrors fn-12's `RunwayObstructionWiring` precedent); migrate three production readers (PilotWiring, ControllerWiring, SimTraceQueries) + the `G3aPilotReactiveCrosswindTest` mutator + 25 direct-constructor test sites. **Firewall surfaces UNCHANGED**: `PilotInput.weatherByAerodrome: Map<AerodromeId, WindReport>` (fn-14.1) keeps its existing shape; `ControllerView.weather: WeatherObservation?` keeps its existing shape — only the *source* changes (readers walk `world.aerodromes` instead of `state.weatherByAerodrome`). The `MissingWeatherForRunwayAerodrome` invariant migrates from `SimState.initial` to an `Aerodrome` invariant.
