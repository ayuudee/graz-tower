...
- Ground geometry: [lowg.dxf](cad/airports/lowg.dxf)
- VFR circuit geometry: [lowg_circuits.dxf](cad/airports/lowg_circuits.dxf)
- Ad hoc airspace geometry: [lowg_airspace_working.dxf](cad/airports/lowg_airspace_working.dxf)
- Normalized airspace geometry: [lowg_airspace_working_normalized.dxf](cad/airports/lowg_airspace_working_normalized.dxf)
- X-Plane baseline geometry: [lowg_xplane_baseline.dxf](cad/airports/lowg_xplane_baseline.dxf)
- Combined working geometry: [lowg_working_combined.dxf](cad/airports/lowg_working_combined.dxf)
- OSM underlay source: [lowg_osm_underlay.geojson](cad/airports/lowg_osm_underlay.geojson)
- OSM underlay geometry: [lowg_osm_underlay.dxf](cad/airports/lowg_osm_underlay.dxf)
- OSM underlay raster: [lowg_osm_underlay.png](cad/airports/lowg_osm_underlay.png)
- OSM underlay placement metadata: [lowg_osm_underlay_placement.json](cad/airports/lowg_osm_underlay_placement.json)
- Authoring manifest: [lowg.manifest.json](cad/airports/lowg.manifest.json)
- Reconciliation report tool: [airport_authoring_report.py](bin/airport_authoring_report.py)
- SVG render tool: [render_airport_authoring.py](bin/render_airport_authoring.py)
- Plate-pack render tool: [render_airport_plate.py](bin/render_airport_plate.py)
- Plate view-model translator: [airport_plate_view_model.py](bin/airport_plate_view_model.py)
- Structured airport-package builder: [airport_structured_package.py](bin/airport_structured_package.py)
- Structured airport-package projection tool: [project_structured_airport_package.py](bin/project_structured_airport_package.py)
- Structured airport-package design note: [structured-airport-package.md](docs/design/structured-airport-package.md)
- Migration-to-core gap handoff: [airport-migration-core-gap.md](docs/design/airport-migration-core-gap.md)
- Entity-bundle projection tool: [project_airport_entities.py](bin/project_airport_entities.py)
- World-candidate projection tool: [project_airport_world_candidate.py](bin/project_airport_world_candidate.py)
- DXF airspace export tool: [export_airspace_dxf.py](bin/export_airspace_dxf.py)
- X-Plane baseline DXF export tool: [export_xplane_baseline_dxf.py](bin/export_xplane_baseline_dxf.py)
- Combined working DXF export tool: [export_airport_working_dxf.py](bin/export_airport_working_dxf.py)
- OSM GeoJSON-to-DXF underlay tool: [export_osm_geojson_dxf.py](bin/export_osm_geojson_dxf.py)
- OSM raster underlay tool: [render_osm_underlay_raster.py](bin/render_osm_underlay_raster.py)
- DXF airspace normalizer: [normalize_airspace_dxf.py](bin/normalize_airspace_dxf.py)
- Entity-fit audit: [lowg-entity-fit.md](cad/airports/lowg-entity-fit.md)
- Render outputs: [rendered/lowg/index.html](cad/airports/rendered/lowg/index.html)
- Plate pack: [rendered/lowg/plate/index.html](cad/airports/rendered/lowg/plate/index.html)
- Structured airport package: [structured-airport-package.json](cad/airports/rendered/lowg/structured-airport-package.json)
- Projected entity bundle: [entity-bundle.json](cad/airports/rendered/lowg/entity-bundle.json)
- Current-core world candidate: [world-candidate.json](cad/airports/rendered/lowg/world-candidate.json)
- Current-core validation report: [world-validation-report.json](cad/airports/rendered/lowg/world-validation-report.json)
- X-Plane airport reference: [LOWG.dat](migration/src/commonTest/resources/airports/LOWG.dat)
- CIFP procedures: [LOWG.dat](data/cifp/LOWG.dat)
- LOWG charts: [LOWG](data/charts/LOWG)
...
- Treat `NEW_*` layers in [lowg_working_combined.dxf](cad/airports/lowg_working_combined.dxf) as collaborative overlays pending promotion into the structured airport package.
...
- The parking-graph interpretation issue is now resolved. The compiler now splits `NEW_Parking` at real line-line intersections and treats direct authored touches onto taxiway `A` as shared graph attachment points rather than forcing everything through the older three projected branch joins.
  - A dedicated entity-fit audit now exists in `cad/airports/lowg-entity-fit.md`. The current conclusion is that LOWG is ready for a version-1 entity projection, with the runtime slice now covering VFR routes, operational sectors, published VFR procedures, runway-specific directional circuits projected from the shared authored graph, and the worked low-level CTR/TMA airspace slice in the current-core candidate. The main remaining gaps are now broader surrounding-airspace projection, altitude-aware airspace membership, and deferred hold semantics.
  ...
  - That bundle now projects LOWG reporting-point routes into the widened runtime `VfrRouteAirspaceProfile` surface. In the current LOWG candidate, `vfr_southeast_entry_path` and `vfr_southwest_entry_path` project as `InVolume(LO585)`, `vfr_western_corridor_path` projects as a segmented `LO585 -> LO0EF_E` route with an explicit boundary transition point, and `vfr_northeast_entry_path` is left explicitly unassigned where any segmented answer would still require invented authority for the pre-CTR leg.
