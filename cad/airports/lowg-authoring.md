...
- The parking-graph interpretation issue is now resolved. The compiler now splits `NEW_Parking` at real line-line intersections and treats direct authored touches onto taxiway `A` as shared graph attachment points rather than forcing everything through the older three projected branch joins.
  - A dedicated entity-fit audit now exists in `cad/airports/lowg-entity-fit.md`. The current conclusion is that LOWG is ready for a version-1 entity projection, with the first runtime slice now covering VFR routes, operational sectors, published VFR procedures, and a worked CTR boundary. The main remaining gaps are directional circuit projection, deferred hold semantics, and broader surrounding-airspace projection.
  ...
  - That clean pass is still explicitly conditional on the synthetic point-claim airspace volume and on the current runway-protection holding-point projection staying on the connected `A` taxiway spine. The remaining real gaps are now directional circuit procedures, deferred hold/loiter modelling, and broader surrounding-airspace projection rather than parking connectivity.
...
2. Treat the current LOWG package and current-core candidate as the version-1 baseline: routes, sectors, published procedures, parking, and the worked CTR boundary are now in.
...
4. Finish directional circuit projection from the shared authored graph into explicit runway/direction-specific `CircuitProcedure` entities.
