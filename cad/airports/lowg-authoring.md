...
- The parking-graph interpretation issue is now resolved. The compiler now splits `NEW_Parking` at real line-line intersections and treats direct authored touches onto taxiway `A` as shared graph attachment points rather than forcing everything through the older three projected branch joins.
  - A dedicated entity-fit audit now exists in `cad/airports/lowg-entity-fit.md`. The current conclusion is that LOWG is ready for a version-1 entity projection, with the first runtime slice now covering VFR routes, operational sectors, published VFR procedures, a worked CTR boundary, and runway-specific directional circuits projected from the shared authored graph. The main remaining gaps are honest airspace membership / route-airspace projection, deferred hold semantics, and broader surrounding-airspace projection.
  ...
  - That bundle now projects LOWG reporting-point routes into the widened runtime `VfrRouteAirspaceProfile` surface, but mixed boundary-crossing routes are currently left unassigned in the current-core projection rather than being forced through invented authority. That is intentionally weaker than the old fabricated uniform-CTR projection, but more honest.
  ...
  - That clean pass is still explicitly conditional on the synthetic point-claim airspace volume and on the current runway-protection holding-point projection staying on the connected `A` taxiway spine. The remaining real gaps are now honest point-to-volume airspace assignment, deferred hold/loiter modelling, and broader surrounding-airspace projection rather than parking connectivity.
...
2. Treat the current LOWG package and current-core candidate as the version-1 baseline: routes, sectors, published procedures, directional circuits, parking, and the worked CTR boundary are now in.
...
4. Assign honest point-to-volume airspace membership for the LOWG worked slice so route-airspace profiles can stop being left unassigned where a fabricated answer would be dishonest.
...
7. Decide how much broader surrounding airspace should be projected into the LOWG current-core candidate beyond the worked CTR boundary and the operational sectors.
