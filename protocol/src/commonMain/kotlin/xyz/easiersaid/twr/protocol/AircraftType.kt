 ...
 *  - Pilot reads [kinematics] + [circuitPattern] directly (sim-internal).
 ...
 *
 * Companion-object constants ([C172], [B738]) are hardcoded; manifest-
 * based loading is filed as **D-AUDIT.4.B-FOLLOWUP**. The runway-length
 * gating rule that consumes [runwayLengthM] is filed as
 * **D-AUDIT.4.A-FOLLOWUP**; multi-type scenario testing is
 * **D-AUDIT.4.D-FOLLOWUP**.
 */
data class AircraftType(
    ...
    /**
     * Per-type kinematic performance. Doctrine sources cited per-constant
     * in the [AircraftType] companion-object KDoc.
     */
    ...
    /**
     * Circuit (traffic-pattern) parameters. GA flies a tighter, lower
     * pattern; jets fly a higher, wider one. Per-type pattern routing
     * in `PilotRoutePlanner` lands with the multi-aircraft pass.
     */
    data class CircuitPattern(
        /** Pattern altitude above aerodrome elevation (m). */
        val altitudeAglM: Double,
        /** Lateral offset from runway centreline on downwind (m). */
    ...
    companion object {
         ...
         *  - FAA AC 90-66: GA standard pattern 1000 ft AGL (305 m),
         *    downwind 0.5 NM offset (~925 m).
         */
        val C172 = AircraftType(
         ...
         *  - FAA AC 90-66 / Boeing FCTM: jet pattern altitude 1500 ft AGL
         *    (457 m), downwind 1 NM offset (~1850 m).
         */
        val B738 = AircraftType(
        ...
        /**
         * Default for fixtures that don't specify a type. Matches G0.
         * Same shape as `ReadbackTimeoutPolicy.Default` (Pass 9) — one
         * named alias for the production-default value.
         */
        val Default: AircraftType = C172
    }
}
