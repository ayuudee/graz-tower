...
- **Golden tests continuously green** — all seven anchors per AGENTS.md: `LowgGoldenTest` (G0, single-aerodrome circuit training), `G1TwoAircraftCircuitsTest` (G1, two-aircraft same-aerodrome circuits=2), `G1TwoAircraftMinimalSpec` (G1-min, two-aircraft circuits=1), `G2CrossAerodromeVfrTest` (G2, LOWG → LJMB cross-aerodrome VFR transit), `G3aPilotTrainedGoAroundTest` (G3a, pilot-trained planned GA), `G3aRunwayObstructionTest` (G3a-obstruction, ATC-instructed reactive GA on world-authored runway obstruction), and `G3aRunwayObstructionContinueApproachTest` (G3a-obstruction-continue-approach, pre-clearance CONTINUE APPROACH on world-authored runway obstruction that clears in time) must remain green at all times; failures are loud, never `@Disabled`.
...
Pure-fold DES engine (`step(SimState, SimEvent) → (SimState, List<SimEvent>)`) with seeded PRNG; obligation-driven BDI controller across TWR / GND / APP / AREA / AFIS roles; pilot agent separate from day one (AI now, human session later); three-layer transmission/reception (physics / interpretation / controller); world model anchored by `path-network-design.md` (entities reference geometry, not lists of node IDs). G0 / G1 / G2 / G3a (pilot-trained GA) / G3a-obstruction (ATC-instructed reactive GA on world-state runway obstruction) / G3a-obstruction-continue-approach (pre-clearance CONTINUE APPROACH on world-state obstruction that clears in time) are the golden anchors. The pre-clearance approach decision space is now **quadruple-covered** end-to-end: clear (`ARR-LAND` direct), continue-traffic (existing `ARR-CONTINUE` for preceding-traffic / runway-access delays), continue-obstruction (`ARR-CONTINUE-APPROACH-OBSTRUCTION` per fn-13 when `RunwayObstructed AND ObstructionClearsInTime`), and go-around (`ARR-GO-AROUND-RUNWAY-OBSTRUCTED` per fn-12 when `RunwayObstructed AND Not(ObstructionClearsInTime)`). Reactive go-around remains triple-covered: self-initiated (pilot-side fn-10 era), pilot-trained (G3a via fn-11 typed `CircuitOutcome.GoAround` outcome), and ATC-instructed-obstruction (G3a-obstruction via fn-12). IFR wiring (IFR-1..6) and approach sequencing are the next live verticals.
