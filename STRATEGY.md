...
- **Golden tests continuously green** — all six anchors per AGENTS.md: `LowgGoldenTest` (G0, single-aerodrome circuit training), `G1TwoAircraftCircuitsTest` (G1, two-aircraft same-aerodrome circuits=2), `G1TwoAircraftMinimalSpec` (G1-min, two-aircraft circuits=1), `G2CrossAerodromeVfrTest` (G2, LOWG → LJMB cross-aerodrome VFR transit), `G3aPilotTrainedGoAroundTest` (G3a, pilot-trained planned GA), and `G3aRunwayObstructionTest` (G3a-obstruction, ATC-instructed reactive GA on world-authored runway obstruction) must remain green at all times; failures are loud, never `@Disabled`.
