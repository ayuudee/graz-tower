...
- **Golden tests continuously green** — `LowgGoldenTest` (G0, single-aerodrome circuit training) and `G2CrossAerodromeVfrTest` (G2, LOWG → LJMB cross-aerodrome VFR transit) must remain green at all times per AGENTS.md; failures are loud, never `@Disabled`.
