...
- **Golden tests continuously green** — all eight anchors per AGENTS.md: `LowgGoldenTest` (G0, single-aerodrome circuit training), `G1TwoAircraftCircuitsTest` (G1, two-aircraft same-aerodrome circuits=2), `G1TwoAircraftMinimalSpec` (G1-min, two-aircraft circuits=1), `G2CrossAerodromeVfrTest` (G2, LOWG → LJMB cross-aerodrome VFR transit), `G3aPilotTrainedGoAroundTest` (G3a, pilot-trained planned GA), `G3aRunwayObstructionTest` (G3a-obstruction, ATC-instructed reactive GA on world-authored runway obstruction), `G3aRunwayObstructionContinueApproachTest` (G3a-obstruction-continue-approach, pre-clearance CONTINUE APPROACH on world-authored runway obstruction that clears in time), and `G3aPilotReactiveCrosswindTest` (G3a-react, pilot-reactive GA off world-authored crosswind exceedance per the aircraft type's POH `maxCrosswindKnots`) must remain green at all times; failures are loud, never `@Disabled`.
