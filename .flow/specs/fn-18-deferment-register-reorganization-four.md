...
1. **`~/.claude/plans/pilot-firewall.md § Deferments register`** (repo-external) — canonical store for ~21 OPEN AUDIT-dotted prefix family/PF prefix family/PASS-prefix family/WORLD-dotted prefix family items from Passes 1-17 and fn-7/fn-8. Not CI-visible. Agents without `Read` on the user's home directory miss the entire register.
2. **fn-14 epic spec § Deferments register** (in-repo, epic-scoped) — 11 PASS-g3a-react-prefix family siblings + `D-PASS-wind-state-migrate-to-aerodrome` + `D-PASS-cap413-edition-24-reconciliation`. Lives in one epic's spec; visible only when reading that epic.
...
**Decided.** Each Kotlin module that has deferments anchored to its API surface gets its own `*DeferredContractsSpec.kt` in `commonTest/kotlin/xyz/easiersaid/twr/<module>/`. Today only `:pilot` has one. Modules likely to grow one at migration time: `:controller` (for D-PASS-fn6-snap-derived, D-PASS-g3a-obstruction-aerodrome-payload, multiple AUDIT-dotted prefix family items pinning controller-side shapes), `:protocol` (for D-PASS-g3a-obstruction-kind-variants, D-PASS-g3a-obstruction-clearsAt-update — these pin `protocol/.../RunwayObstruction.kt`'s shape), `:sim` (for D-PASS-fixture-per-plan-filing-time — pins `Fixture.kt`'s shape), and `:core` (likely empty in v1 — most `:core` deferments are FM/Lean-territory ones owned by `D-AUDIT-polygon-ctr` which goes to bucket 4).
...
- **`planned`** — has a flow-next epic stub (`todo` status) whose tasks are NOT all `done`. Bucket 3 entries. **Verify at fn-18.3 time** (per plan-review round 13 — codex finding "closed spec-sourced IDs misclassified"): check the epic's task statuses; if ALL tasks are `done` the deferment is CLOSED and migrates to `## Archive`, NOT bucket 3 active. At fn-18 planning time (post-this-session closure): fn-16 is the canonical bucket-3 example. fn-15 and fn-17 each had `todo` epics at the brief-writing date but were closed during this session — their associated D-* entries (`D-PASS-g3a-react-tailwind-limit`, `D-PASS-cap413-edition-24-reconciliation`) MUST be Archive entries, not planned.
- **`narrative`** — bucket 4 entry with no clear pin today. May graduate to `blocked` or `planned` later.
- **`closed`** — kept ONLY in `## Archive` section for history (per plan-review round 12 — codex finding "two canonical locations for closed entries"). Closed entries do NOT appear in the active body; they live exclusively in `## Archive` and are moved there at closure time. Previously this leaf said "Struck through or marked CLOSED in the active section" — that wording is removed because it created an ambiguity where a closed entry could exist in both places. Decision #6 (bucket transition) governs the in-place status move: when a deferment closes, its `### D-...` heading + body is cut from the active prefix section and pasted into `## Archive` (with body trimmed to the three locked archive fields per Decision #3). Single canonical location, no duplicates.
...
**Contract:** <conditional — present when the pre-migration source body carried a real-fix contract richer than 3 sentences of `Why:` can hold. Captures the eventual API shape, blocked-on prerequisite, named closure trigger, or test-that-lands-when-closed. Verbatim from source. Per plan-review round 14 — codex finding "rich .plan D-* bodies lose contract detail when Why: caps at 3 sentences".>
**Closes by:** <new epic when activated | inline in epic fn-N | archived when prerequisite lands | epic fn-N (planned)>
```

Field order is fixed. `Blocked on:` is omitted entirely (not blank, not "n/a") when `Status ≠ blocked`. `Contract:` is similarly omitted when the source body fits inside the `Why:` field's 1-3 sentence cap (i.e. it's a short doctrinal note, not a rich real-fix contract). Other fields are always present. The 1-3 sentence cap on `Why:` is enforced by review; `Contract:` has no length cap because it's preserving primary-source content that would otherwise be lost.
...
- **`docs/deferments.md`** stays the canonical register for: named `D-*` deferments (`PF prefix family`, `AUDIT-dotted prefix family`, PASS-prefix-family, `WORLD-dotted prefix family`) — work consciously parked with a real-fix contract (the eventual API shape, the missing prerequisite, the test that lands when it's closed). These are deferred contract/spec commitments — what we promised about the future when we shipped something incomplete.
...
**Handling** (per plan-review round 10, hardened per plan-review round 17 — codex finding "Decision #10 still permits losing rich .plan contracts when fn-18.2 already filed the ID"): fn-18.3 greps `.plan` for `D-*` IDs explicitly at task start. **Content-preservation audit is MANDATORY for every `.plan` D-* entry, including ones already filed by fn-18.2** — see fn-18.3 Step 1 for the audit rule. The audit compares the `.plan` body against the corresponding `docs/deferments.md` entry; any contract detail (eventual API shape, blocked-on prerequisite, named closure trigger, regression-test plan) present in `.plan` but missing from docs MUST be merged into `Why:` / `Contract:` / `Closes by:` BEFORE rewriting `.plan`. For each ID:
- If status in `.plan` is OPEN and the ID is also in pilot-firewall.md OPEN list (handled by fn-18.2): **content-preservation audit FIRST** (merge missing `.plan` body detail into the docs entry), then no new docs entry is created — fn-18.2 already created it and fn-18.3 augmented it.
- If status in `.plan` is OPEN and the ID is NOT in pilot-firewall.md or epic specs: bucket-triage and file a new active entry in `docs/deferments.md`, importing the `.plan` body verbatim into `Why:` + `Contract:` per the schema.
- If status in `.plan` is DONE / CLOSED-PARTIAL: file as an Archive entry (Status + Closed-by + Enforcement per Decision #3).
- The `.plan` D-* line IS rewritten by fn-18.3 (hardened per plan-review round 13 — codex finding "two active-looking canonical surfaces") to a one-line pointer `<ID> — see docs/deferments.md`. This collapses the previously-ambiguous duplicate-surface state into a single canonical record. Non-D-* `.plan` content (the operational backlog: B*, IFR-*, RR-*, M*, narrative) is **NOT** edited — that's the user's working surface. The cross-reference is one-way: `docs/deferments.md` is the canonical for D-* IDs; the `.plan` pointer is a discovery breadcrumb for grep'ers landing in `.plan` first.
...
**Decided.** `/Users/andrew/.claude/projects/-Users-andrew-dev-projects-graz-tower/memory/reference_audit_registers.md` becomes stale after fn-18 ships. The acceptance criterion (R10) is that this memory entry is rewritten to:
...
Header **template** (revised per plan-review round 11 — codex finding "MIGRATED header `<pending>` SHA", normalized per plan-review round 14 — codex finding "epic Decision #12 vs fn-18.2 Step 8 header text mismatch"): no `<pending>` placeholder the agent cannot update; replace with a durable `git log --grep` pointer instead. Single canonical text shared between epic Decision #12 and fn-18.2 Step 8:
...
- **fn-18.3 — Migrate fn-14/15/17 epic-spec siblings + inline-only IDs.** 13 fn-14 siblings + 6 fn-15 siblings + 3 fn-17 siblings + ~6 inline-only IDs. **Bucket-3 vs Archive classification per epic-status verification** (hardened per plan-review round 13): only fn-16 (tasks still `todo`) gets a bucket-3 active entry. fn-15 and fn-17 closed during the fn-18-execution session — their `D-PASS-g3a-react-tailwind-limit` and `D-PASS-cap413-edition-24-reconciliation` entries go to `## Archive` with `Closed by:` + `Enforcement:` pointers. fn-14 epic spec's `## Deferments register` section gets updated to point at `docs/deferments.md` as the single source of truth (fn-15 / fn-17 specs similarly).
...
- **R4** — `/Users/andrew/.claude/projects/-Users-andrew-dev-projects-graz-tower/memory/reference_audit_registers.md` rewritten per Decision #11. Two distinct sub-criteria (split per plan-review round 7 — original criterion conflated repo-side and external-side actions, same shape as R11):
...
- **R9** — Bucket-3 epic stubs confirmed cross-referenced **only when actually open** (per plan-review round 13). At fn-18 close, the genuinely-bucket-3 set is **fn-16** (tasks `todo`). fn-15 and fn-17 had `todo` task statuses at the brief-writing date but were closed during the fn-18-execution session — their associated `D-PASS-g3a-react-tailwind-limit` and `D-PASS-cap413-edition-24-reconciliation` entries migrate to `## Archive`, NOT bucket 3 active. Any new bucket-3 epic stubs created during migration (if a pilot-firewall.md item turns out to need multi-task scope) are created via `.flow/bin/flowctl epic create` and set to `todo` status before the docs entry is written. fn-18.3's locked inventory step verifies each epic's task statuses (`flowctl show <epic-id> --json | jq '.tasks | map(.status)'`) before classifying as `planned`.
...
- **R15 (final whole-repo exhaustiveness gate, added per plan-review round 7)** — at fn-18.3 close, run a **whole-repo grep** across `*.md` (specs, tasks, design docs, repo-root markdown), `*.kt` (all modules), and `.plan` for every `D-*` ID pattern (`D-PASS-`, `D-AUDIT.`, `D-AUDIT-`, `D-PF.`, `D-WORLD-`, `D-WORLD.`). Compare the de-duplicated ID set against `docs/deferments.md`'s set of `### D-...` headings. Every concrete ID found in the whole-repo grep MUST appear exactly once in `docs/deferments.md` (active body or `## Archive`). Documented placeholder IDs (e.g. `D-PASS-cap413-edition-24-<section>`) are excluded by virtue of being placeholders, not stable IDs. The de-dup-and-diff result is recorded in fn-18.3's done summary; any drift fails R15. This is the final exhaustiveness gate that catches any ID neither fn-18.2 nor fn-18.3's per-source inventory captured — meta-defence against inventory rot.
  ...
  **Hardened gate semantics** (per plan-review round 10, extended per epic-review round 1 — codex completion-review finding "prefix-fragment regex artifacts from narrative cross-references to canonical IDs"): `comm -23` non-empty is allowed at task close ONLY for proven false positives — specifically (i) regex artefacts where a placeholder slipped through the tail-char `[A-Za-z0-9_]` exclusion (in which case fix the placeholder text in-place, not the gate), (ii) IDs that appear only inside a `<...>`-fenced CONVENTION schema example (in which case fix the CONVENTION text to use a more obviously-placeholder form), OR (iii) **prefix-of-canonical-ID regex artefacts** — where the greedy `[A-Za-z0-9_.-]*[A-Za-z0-9_]` regex extracts a strict prefix substring of a real canonical ID that DOES have a `### D-...` heading in `docs/deferments.md`. Class (iii) is mechanically auditable: every fragment in this class must be a strict prefix of some `### D-` heading in docs. If `comm -23` lists an ID `D-PASS-foo` and `docs/deferments.md` has a heading `### D-PASS-foo-bar-baz`, the missing ID is a prefix-of-canonical artifact and excluded. The done-summary R15 result records each exclusion's class (i/ii/iii) with the canonical ID it maps to. The "explained line-by-line in done summary with rationale" allowance is NOT a general escape hatch — every legitimate concrete repo-wide ID MUST be added to `docs/deferments.md` before task close. If the missing-ID list contains a real ID that has no docs entry AND is not a prefix of a canonical ID, the task FAILS R15 — add the entry. Per `feedback_no_corners.md`: no silent prose-pardons of missing entries.

- **R16 (bucket-1 value-flow acceptance, added per plan-review round 7, scope-widened per plan-review round 16 — codex finding "R16 covers only NEW tests; retained bucket-1 placeholders escape compile-check")** — every `@Ignore`d test referenced by a bucket-1 entry in `docs/deferments.md`, **whether newly written or retained from a pre-fn-18 placeholder**, contains at least one **non-import** real-current-API value-flow reference inside the test body (per epic Decision #1, hardened per plan-review round 6 — "import-only references do NOT count"). The reference must be: an assertion using a real type, OR a type construction of a real domain class, OR a function call returning a real value. Imports-only fails R16. Verified at task close by inspecting each bucket-1 test body referenced by `docs/deferments.md` (read each `Pinned at: <file>::<test>` value, open the test, check value-flow); result recorded in the respective task's done summary. **For retained placeholders (D-PF.1 / D-PF.3 + any other bucket-1 candidate kept from the pre-fn-18 `DeferredContractsSpec.kt`)**: if the retained test body is import-only, fn-18.2 either (a) re-classifies the deferment as bucket 2 in `docs/deferments.md` (status unchanged, but `Pinned at:` updates to acknowledge commented-out shape), or (b) adds the missing value-flow reference in the test body as part of the same commit. Silent retention of an import-only "bucket 1" placeholder is forbidden. Bucket-2 tests are exempt (commented-out future API by design).
...
- **R14** — Test suite outcome recorded after each task. Two explicit outcomes (per plan-review round 6 — internal-consistency fix on R14 semantics):
  - **R14-Passed** — `./gradlew :pilot:jvmTest :controller:jvmTest :protocol:allTests :sim:jvmTest :core:allTests detekt` exits 0. Nine goldens GREEN. Detekt baseline unchanged. This is the desired path.
...
2. **Task .2 — Migrate pilot-firewall.md OPEN items + orphan-test cleanup.** ~32 items triaged across the four buckets:
    ...
    - User-performed pilot-firewall.md "MIGRATED" header — fn-18.2 provides the exact header text and logs the external-follow-up status (`pending` by default; `confirmed-by-user` ONLY if the user independently reports they have applied the edit, per plan-review round 7 normalization). fn-18.2 NEVER auto-flips to `confirmed`; the agent has no way to observe the external edit.

3. **Task .3 — Migrate fn-14/15/17 epic-spec siblings + inline-only IDs.** 22 spec-sourced IDs + ~6 inline-only IDs:
    - fn-16 (only genuinely-open epic stub) gets a `docs/deferments.md` bucket-3 active entry pointing at the existing stub. fn-15 and fn-17 closed during the fn-18-execution session — their `D-PASS-g3a-react-tailwind-limit` and `D-PASS-cap413-edition-24-reconciliation` entries go to `## Archive`, NOT bucket-3 active (per plan-review rounds 13-14).
...
| Flow-next epic stub | fn-16 (genuinely open, tasks `todo`). fn-15 and fn-17 had `todo` tasks at brief-writing but were closed during this session — those are Archive entries, not bucket 3. | None new unless a pilot-firewall item warrants escalation |
...
| Inline `// PASS-prefix family` comments | ~30 sites across `:pilot`, `:controller`, `:protocol`, `:sim`, `:core`, `:migration` | UNCHANGED — these stay as breadcrumbs; cross-references updated in docs map only |
...
**Task fn-18.1** validates the convention by writing the empty `docs/deferments.md` and `docs/deferments-CONVENTION.md`. After .1 lands, a reader following the convention doc and looking at the empty map should be able to predict: "the next entries I see will be the PF prefix family / AUDIT-dotted prefix family items currently in `~/.claude/plans/pilot-firewall.md`, with `Pinned at:` field values pointing at either `DeferredContractsSpec.kt`, flow-next epic IDs, or `narrative only`." If a reviewer reads the convention doc and can't predict the shape of what fn-18.2 will produce, the convention is unclear and .1 fails review.
...
- `feedback_firewall_principle.md` — the pilot firewall is first-class; PF prefix family items track its evolution. fn-18 makes those items CI-visible.
...
- `.flow/specs/fn-16-wind-state-migrate-to-aerodromeweather.md` — already-existing bucket-3 epic stub (tasks `todo`). `.flow/specs/fn-15-g3a-react-tailwind-pilot-reactive-go.md` and `.flow/specs/fn-17-cap-413-edition-24-numbering.md` were bucket-3 candidates at brief-writing time but closed during this session — their D-* deferments migrate to Archive, not bucket 3.
...
| R9  | Bucket-3 epic stubs created (where needed) before docs entries written; epic-status-verified at task time so only genuinely-open epics get `planned` (closed epics go to Archive) | fn-18.2 (if any new), fn-18.3 (existing) |
...
| R14 | Recorded as **R14-Passed** (gradle + detekt exit 0; nine goldens GREEN; baseline unchanged) OR **R14-NoNewRegression** (baseline was already red; task evidence proves no new failures introduced). Each task records one. | fn-18.1, fn-18.2, fn-18.3 (each task verifies) |
...
| R16 | Every bucket-1 `@Ignore`d test referenced by `docs/deferments.md` (new OR retained) contains a non-import current-API value-flow reference; retained import-only placeholders are either upgraded to bucket-1-by-value-flow or re-classified to bucket 2 | fn-18.2, fn-18.3 |
