...
**Filename convention** (hardened post-implementation per completion-review round 1 — codex finding "specified unsuffixed paths but flowctl memory requires dates"): `flowctl memory` requires `<slug>-YYYY-MM-DD.md` filenames for entry registration. R1-R6 paths in this spec are short-form (e.g. `test-pin-discipline.md`); the on-disk filenames carry the `-2026-05-15` date suffix (the task-execution date). MEMORY.md index entries point at the dated filenames; cross-references between knowledge entries also use the dated form. The unsuffixed names in R1-R6 are spec convention only — they refer to the same entries.
...
- **R1:** New entry `.flow/memory/knowledge/best-practices/test-pin-discipline-2026-05-15.md` (dated filename per flowctl memory convention; spec short form `test-pin-discipline.md`) — synthesizes the 4 test-pinning lessons (mint-id vs tx-start timestamps, observe-real-post-state not predicate compounds, allowlists key by stable identifier, compound-predicate failure modes). Cross-references the 4 source bug captures.
- **R2:** New entry `.flow/memory/knowledge/best-practices/inherited-gate-semantics-2026-05-15.md` (dated filename per flowctl memory convention; spec short form `inherited-gate-semantics.md`) — copy-pasted sim-test gates between sibling axes need re-validation per semantic meaning, not syntactic pattern (the fn-15.2 lesson where the off-final gate semantically meant "back on downwind" by coincidence on crosswind axis but didn't transfer to tailwind).
- **R3:** New entry `.flow/memory/knowledge/conventions/rich-world-domain-2026-05-15.md` (dated filename per flowctl memory convention; spec short form `rich-world-domain.md`) — time-varying state lives on the entity it concerns (`Aerodrome.weather`, `Runway.obstruction`); hard atomic cutover, no shims; no `SimState.*-by-aerodrome` flat maps when an entity exists. Cross-references fn-12 and fn-16 as precedents.
- **R4:** New entry `.flow/memory/knowledge/best-practices/renumbering-grep-walk-2026-05-15.md` (dated filename per flowctl memory convention; spec short form `renumbering-grep-walk.md`) — when reconciling renumbering across docs, the grep walk must span the FULL §-number range affected by the edition's shift, not just the hypothesis's focal sections.
- **R5:** New entry `.flow/memory/knowledge/best-practices/pre-existing-failure-register-2026-05-15.md` (dated filename per flowctl memory convention; spec short form `pre-existing-failure-register.md`) — pre-existing test failures carried across epics must be promoted to named `D-WORLD.N` (or similar prefix-family) register entries in `docs/deferments.md`; carve-out narrative across multiple epic specs is a smell, not a pattern.
- **R6:** New entry `.flow/memory/knowledge/workflow/flowctl-done-state-sync-2026-05-15.md` (dated filename per flowctl memory convention; spec short form `flowctl-done-state-sync.md`) — long-spec flow-next tasks where `flowctl done` writes its own canonical summary into the task spec md AND prior worker-block narrative leaks into the same file cause codex impl-review to cycle repeatedly on "spec md state doesn't match implementation state." Each round surfaces a different artifact mismatch (BLOCKED narrative, empty Evidence block, `.json` status=todo, set-cardinality inconsistency). Forward-applicable rule: after `flowctl done`, run a post-done state-synchronization sweep — clear stale narrative, populate Evidence, flip `.json` status — before re-invoking codex impl-review. Source: `.flow/memory/bug/build-errors/long-spec-flow-next-tasks-review-loop-2026-05-13.md` (verified per plan-review round 1).
...
- **R8:** `flowctl memory search "<theme>" --limit 5 --json` returns the new knowledge entry in the **top-3** hits for each theme (e.g. `search "mint-id"` → test-pin-discipline; `search "rich-world-domain"` → rich-world-domain; etc.). Verified per-entry at task close. **Note** (relaxed from "top hit" per completion-review round 1 — codex finding "evidence claims top-3, not top hit"): the bug/* captures legitimately use theme tokens directly in their titles, so they often win top hit on the search; the goal is discoverability (top-3 is sufficient to surface alongside the original symptom record), not displacement.
...
- **R10:** Diff scope: 6 new files under `.flow/memory/knowledge/` + 1 modified `.flow/memory/MEMORY.md` = 7 files of primary work, PLUS any post-implementation spec amendments to `.flow/specs/fn-22-*.md` that flow from completion-review findings (the post-hoc dated-filename clarification per round 1 is such an amendment). No production code, no tests, no docs outside `.flow/memory/` or `.flow/specs/`. Spec amendments are part of the review-loop discipline; not a scope violation.
