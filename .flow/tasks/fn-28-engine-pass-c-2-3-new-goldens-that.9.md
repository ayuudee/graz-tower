...
- [ ] **`PilotEvent.AbortTakeoff` sealed leaf added** (round-15 Major 1): fields needed for trace/debugging (e.g., `aircraftId`, `time`, `speedAtFailure`); `pilotDecide`'s exhaustive `when` dispatches it. Final R21 order in `derivePilotEvent` finalized: `DecisionAltitudeWithoutClearance → DensityAltitudeDecline → AbortTakeoff → TailwindLimitExceeded → CrosswindLimitExceeded`. KDoc on `derivePilotEvent` updated
