...
The remaining ~23 inline-only IDs (regex-fragment matches against longer IDs, plus closed/CLOSED-PARTIAL items surfaced by inline grep) are listed verbatim in fn-18.2's `set_inline_only_for_fn_18_3`. fn-18.2's evidence also flags ``(WORLD-BACKED-false-positive)`` as a known false positive (ClearanceId string literal in `core/.../ResolvedClearanceTest.kt:343`) and notes `D-PASS-g3b-react-cross`, `D-AUDIT.9.II`, `D-AUDIT.2.A`/`.B`/`.E` as regex-fragment matches against longer IDs — fn-18.3 de-dups these during its own reconciliation pass.
...
- Sources (overlapping — IDs surface in multiple sources; per-source counts do NOT sum to 74):
  ...
  - fn-18.2 inline-only handoff: 29 IDs minus 6 false-positive fragments per fn-18.2 evidence (`D-PASS-g3b-react-cross`, `D-AUDIT.9.II`, `D-AUDIT.2.A`, `D-AUDIT.2.B`, `D-AUDIT.2.E`, ``(WORLD-BACKED-false-positive)``) = **23 real concrete IDs**, decomposing as 5 archive (D-PASS-13.1/13.2/13.3 Pass-17 closures + D-PASS-g3a-react-tailwind-limit fn-15-archive + D-PASS-wind-state-migrate-to-aerodrome bucket-3) + 18 active narrative
...
- `docs_missing_ids_check`: 14 entries remain in the `comm -23 repo-ids docs-ids` output. Each is a regex-greedy-extraction artifact against a canonical longer ID that DOES have a docs entry. **Durable inline materialisation** of the missing-IDs list (each entry pairs the extraction artifact with the canonical longer ID and the source line that produced the match):
