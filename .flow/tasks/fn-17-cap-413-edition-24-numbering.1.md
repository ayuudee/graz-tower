...
## Evidence
- Commits (oldest → newest, all full SHAs, all reachable from HEAD `e2f1e1ee7a6e94a4bd0ae053eda70c1929d1bf7e`):
  1. `68dc375a02b3b2fde95d44bd74238377e648cfbb` — primary implementation (Branch A rename sweep + CAP_413_EDITION constant + capture artifact + closed-epic errata footers + .plan deferment closures)
  2. `3bfe9ced19488415a392764da09dfef58bc551da` — R11 verify completed (eight goldens GREEN under cloned-GRADLE_USER_HOME workaround; D-PASS-cap413-edition-24-r11-verify-sandbox-block filed + closed)
  3. `9cc2af2af602b20ec96e86086fb8f9facd8d89dd` — populate Done summary + Evidence sections (R10b initial pass)
  4. `12344c56c9f4954fa5dff4919a087ee800a76e9a` — codex round-3 fixes (stale §4.55-4.56 prose at Controller.kt:917 + AGENTS.md:281; errata-footer scope extension on fn-12/fn-13 for §4.53/§4.55/§4.56 mappings; R10b sequencing clarification)
  5. `e2f1e1ee7a6e94a4bd0ae053eda70c1929d1bf7e` — flowctl done + memory capture (`.flow/memory/bug/build-errors/renumbering-grep-walk-must-span-full-2026-05-11.md`)
- Register-touch list (R10):
  - `.plan` — D-PASS closures: `cap413-edition-24-reconciliation` (filed fn-14, closed by fn-17.1 Branch A verdict), `cap413-edition-24-r11-verify-sandbox-block` (filed + closed within fn-17.1). New OPEN deferments: `D-PASS-cap413-2_7-principle-cite-audit` and `D-PASS-cap413-4_46-principle-cite-audit` (pre-existing principle-vs-cite drift surfaced during Table 2 audit, out of fn-17 renumbering scope).
  - `~/.claude/plans/pilot-firewall.md` — sister register **does not exist** on this machine (fn-18 will establish the `docs/deferments.md` canonical-map sister convention); no touch required.
  - `docs/deferments.md` — does not yet exist; fn-18 introduces the four-bucket model. fn-17 deferments live in `.plan` per current canon.
  - `.flow/specs/fn-14-g3a-react-pilot-reactive-go-around-on.md` — inline updates (line 22 + line 154 references + line 404 deferment closure marker) per R7 Branch A.
  - `.flow/specs/fn-11-…md`, `.flow/specs/fn-12-…md`, `.flow/specs/fn-13-…md` — `## Errata` footers appended with complete §-number mappings (R8 Branch A).
  - `.flow/specs/fn-15-…md` — minor cross-reference touch; no semantic change to fn-15 scope.
  - `.flow/memory/bug/build-errors/renumbering-grep-walk-must-span-full-2026-05-11.md` — added (round-3 lesson: a renumbering grep walk must span the FULL §-number range affected by the edition's shift, not just the hypothesis's focal sections).
  - `wiki/data-sources/cap413-edition-24-capture.md` — added (R1 verification artifact: Tables 1+2, mapping, retained historical cites, local extraction procedure).
- Grep audit (R10c, captured 2026-05-11 against HEAD `e2f1e1e`):
  - `git grep -nE '§4\.(65|66|67|68|49|53|55|56)'` excluding `.flow/{specs,tasks,memory}`, `.plan`, `wiki/data-sources/cap413-edition-24-capture.md`, and `research/` → **0 matches** in current-doctrine surfaces. All retained references live in deliberately-historical contexts (errata footers in closed-epic specs, audit-history in fn-17 task spec/memory, deferment classification in .plan, the capture artifact itself).
  - `git grep -n '27th ed'` excluding `.flow/*`, `.plan`, `wiki/…`, `research/` → **0 matches**. The wrong literal is fully eliminated from active code paths.
  - `grep -rnE 'CAP413_4_65\b' --include="*.kt"` → only KDoc/comment lines that explicitly mention the rename ("`CAP413_4_65` renamed to `CAP413_4_64`" / "formerly `CAP413_4_65` in Ed 23"). No live consumer references remain.
  - `grep -rnE 'CAP413_4_64' --include="*.kt"` → present in `RegulationDatabase.kt:545` (the typed entry) and `Controller.kt:933` / `TowerArrival.kt:83,267` (consumers); rename is compiler-coherent.
  - `grep -nE 'edition = "Edition 23' protocol/.../RegulationDatabase.kt` → exactly two retained inline literals at lines 373 (`CAP413_2_7`) and 393 (`CAP413_4_46`), gated by the two new OPEN deferments. Citation-triple coherence preserved (Ed-23-only principle content keeps Ed-23 metadata; all other CAP413_* entries use the `CAP_413_EDITION = "Edition 24 (effective 1 July 2026)"` constant).
- Tests (R11): `./gradlew :sim:jvmTest :pilot:jvmTest :controller:jvmTest :core:allTests :protocol:allTests detekt --offline --no-daemon` — BUILD SUCCESSFUL in 40s (25 actionable tasks executed). Eight goldens GREEN per per-class JUnit XML verification at `sim/build/test-results/jvmTest/TEST-xyz.easiersaid.twr.sim.{LowgGoldenTest,G1TwoAircraftCircuitsTest,G1TwoAircraftMinimalSpec,G2CrossAerodromeVfrTest,G3aPilotTrainedGoAroundTest,G3aRunwayObstructionTest,G3aRunwayObstructionContinueApproachTest,G3aPilotReactiveCrosswindTest}.xml` — each: `failures="0" errors="0" skipped="0" tests="[1-9][0-9]*"`. Sandbox workaround (cloned `GRADLE_USER_HOME=$TMPDIR/gradle-user-home` + `_JAVA_OPTIONS=-Djava.io.tmpdir=$TMPDIR` + Nix Zulu JDK 21) documented in `.plan` under `D-PASS-cap413-edition-24-r11-verify-sandbox-block` (CLOSED) for future implementers in similarly-restricted sandboxes.
- PRs: (none — landed directly on `main`; the eight commits ahead of `origin/main` pre-dating fn-17 plus the five fn-17 commits will ship together when the user pushes.)