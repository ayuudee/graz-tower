...
| Route/vector control delivered surface: `ProceedDirect`, `LeaveHoldProceedDirect`, `WhenAbleProceedDirect`, `RejoinSidAt`, `JoinAirway`, `ResumeOwnNavigation`, `RouteAsFiled`, `FlyHeading`, `TurnHeading`, `TurnByDegrees`, `ContinuePresentHeading`, `StopTurn`, `InterceptLocaliser`, plus the first narrow immediate-adjunct compound layer | `ResolvedClearance.kt`, `CompletionEvaluation.kt`, current route-control rules in `InstructionRules.kt` | `GreenfieldRouteControlCurrentShape`, `GreenfieldRouteControlCompound`, `GreenfieldRouteControlDeliveredCurrentShape` | `CURRENT_SHAPE_COMPLETE` | Direct-fix / airway-join completion is closed on the current published-world model; heading/vector instructions now resolve through explicit vector payloads with issue-time heading capture where required; `TurnByDegrees` is closed on the current explicit observed-turn-progress model; richer heading-hold semantics remain current-shape only. |
