...
This file is the current planning document for `research2`.
...
- a concrete pairwise checker and `SeparationCheckerSoundnessTheorem` now exist
  in [SeparationChecker.lean](/home/andrew/dev/projects/twr/research2/lean/CertifiedAtc/SeparationChecker.lean)
...
- a conservative concrete neutral-command slice now exists for the current
  ...
  no-ops
- the project now has an explicit compiled app-to-certifier view model in
  [CertifierViews.kt](/home/andrew/dev/projects/twr/core/src/commonMain/kotlin/dev/twr/core/model/CertifierViews.kt);
  with the April 2026 pivot to `greenfield/`, that shape should now be treated
  as provisional staging rather than as the default repo-local implementation
  target
...
- the project now has a greenfield clearance-boundary scaffold in Kotlin:
  entity-referenced instructions, `ProcedureRef`, compound clearance content,
  `StructuredClearance`, and `ClearanceCompileView`
