...
- Impact: COMMS-1 has moved to covered-green via fn-50, the contact-frequency phraseology row moved to rendered phraseology via fn-78, the readback-termination row is split by fn-81 for supported rendered readback templates, the TAKE OFF word-use row is split by fn-82 for supported rendered controller templates, and the §2.8.1 full-callsign / ALL STATIONS examples moved to synthetic rendered-example coverage via fn-89; remaining entries are phraseology, policy, model-gap, or not-applicable records.
