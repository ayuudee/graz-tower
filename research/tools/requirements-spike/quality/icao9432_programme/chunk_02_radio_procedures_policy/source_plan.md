...
| `icao9432-extracted::test_procedures_2_8_4_en::d67d1f63cbbecd7d` | `test_procedures_2_8_4_en` | `split`: duration `covered-green` / content `phraseology-later` | `PHRASE-1` for content | 4086-4197 | accepted / quote pass | Ground-station test-signal duration is limited to 10 seconds; content must be spoken numbers followed by station callsign. fn-52.3 covers duration only with typed signal identity and measured transmission start/end evidence. |
...
- The engine-start-after-approval unit `95034efc191fa9cd` remains a candidate
  for D-PF.1. fn-52.2 confirmed current events cannot honestly observe both
  approval and subsequent start without adding the real startup lifecycle.
- The radio-test-signal unit `d67d1f63cbbecd7d` is split: fn-52.3 covers the
  duration sub-obligation with typed signal identity and measured
  start/end-derived duration; spoken-number and callsign content remains
...
| `icao9432-extracted::aerodrome_ch4_intro_start_4_1_to_4_2_en::95034efc191fa9cd` | `expected-gap` blocked on D-PF.1 | Protocol has `StartupApproved`, and the pilot mission can complete `AWAIT_STARTUP_APPROVAL`, but the live departure tree deliberately omits startup clearance under D-PF.1 and no source-mapped evidence fact proves the workflow "approval then engine start assisted as necessary". fn-52.2 confirmed that default `engineRunning == true` is not valid engine-start evidence. |
...
| `icao9432-extracted::test_procedures_2_8_4_en::d67d1f63cbbecd7d` | `split`: duration `covered-green` / content `phraseology-later` | fn-52.3 added typed `GroundStationTestSignal` identity, mandatory transmission end times in `TransmissionRecord`, and a source-mapped duration audit citing ICAO 9432 §2.8.4.4 / `d67d1f63cbbecd7d`. Spoken-number and callsign content remains `PHRASE-1`. |
